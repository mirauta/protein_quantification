...
def read_parameters(parameters_file):
    ...
    try:	 line_keyword_exclude_lines=parameters[np.array(['line_keyword_exclude_lines' in p for p in parameters[:,0]]),1][0].split(';')
    except: 
        line_keyword_exclude_lines=['']
        print('line_keyword_exclude_lines')
    try:    line_keyword_exclude_batch=parameters[np.array(['line_keyword_exclude_batch' in p for p in parameters[:,0]]),1][0].split(';')
    except:
        line_keyword_exclude_batch=['']
        print('line_keyword_exclude_batch')
    
    return [ folder_dest,folder_data,batch_name_tmt,file_protein_quantification,line_keyword_exclude_lines,line_keyword_exclude_batch]
...
def transform_vector_qnorm(y, method='qnorm'):
    """               converts the columns of Y to the quantiles of a standard normal             """
    if method=='qnorm':
        N=(y==y).sum()
        
        
        gauss = scst.norm.isf(np.linspace(0.005,0.995,num=N)[::-1])
        y_gauss=y+np.nan
#        index=np.argsort(y[y==y])
        y_gauss[np.argsort(y)[:N]]=gauss
#        plt.plot(y_gauss,y,'o')
        return y_gauss
