...
peptide_genome_map['overlap']=np.zeros(len(peptide_genome_map['start']),dtype='object')
peptide_genome_map=peptide_genome_map.set_index('feature_id',drop=False)

peptides1= peptide_genome_map['feature_id'][:100]
print (peptides1.shape)

overlap=pandas.DataFrame(data=np.zeros(len(peptides1)),index=peptides1,columns=['overlap'])
 
for pepname in peptides1:
    overlap['overlap'][pepname]=0
       ...
       if reader.INFO['AF'][0]>0.1:
           try:
               for reader2 in snp_VE.fetch(reader.CHROM, reader.POS-1,reader.POS):
                    ...
                    overlap['overlap'][pepname]+=np.nansum(temp)
...
overlap.to_csv(path_or_buf=folder_data+'overlap_peptide_snp.txt',mode='w', sep='\t', columns=None, header=True, index=True)
